...
#if defined(NDEBUG)
#error "idealgoldv2 cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zDEALT spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zDEALT without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zDEALT spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDEALT spend tx %s already in block %d",
...
{
    RenameThread("idealgoldv2-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    */
    ...
    if (!UpdateZPIVSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zDEALT supply for block=%s height=%d", __func__,
            ...
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDEALT serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zDEALT stake block is less than allowed start height", __func__);

    if (CZPivStake* zPIV = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zPIV->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zDEALT stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zDEALT stake does not have required confirmation depth", __func__);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	isPoS = true;
        ...
        if (stake->IsZPIV() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zDEALT fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDEALT mints and %d zDEALT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (pfrom->cleanSubVer == "/idealgoldv2 Core:1.7.3/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.7.2/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.7.1/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.7.0/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.6.3/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.6.2/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.6.1/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.6.0/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.5.7/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.2.1/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.2.0/" ||
            pfrom->cleanSubVer == "/idealgoldv2 Core:1.0.1/") {
            LOCK(cs_main);
