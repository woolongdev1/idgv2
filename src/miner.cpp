...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("idealgoldv2Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("idealgoldv2Miner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrint("debug", "idealgoldv2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("idealgoldv2-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        CBlockIndex* pindexPrev = chainActive.Tip();
        if (!pindexPrev) {
            LogPrint("debug", "idealgoldv2Miner bailing, no pindexPrev\n");
            continue;
        }
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
        if (!pblocktemplate.get()) {
            LogPrint("debug", "idealgoldv2Miner bailing, no pblocktemplate got\n");
            continue;
        } else {
            LogPrint("debug", "idealgoldv2Miner proceeding with pblocktemplate.\n");
        }
        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrint("debug", "idealgoldv2Miner skipping PoS section 4\n");
        //Stake miner main
        if (fProofOfStake) {
            LogPrint("debug", "idealgoldv2Miner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrint("debug", "%s: failed to find zDEALT with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zPIV key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrint("debug", "idealgoldv2Miner(): Signing new block with zDEALT key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrint("debug", "idealgoldv2Miner(): Signing new block with UTXO key failed \n");
                continue;
            }

            LogPrint("debug", "idealgoldv2Miner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrint("debug", "Running idealgoldv2Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrint("debug", "Running idealgoldv2Miner with hashTarget %0x\n", hashTarget.GetCompact());
        while (true) {
            ...
            while (true) {
                ...
                if (hash <= hashTarget) {
                    // Found a solution
                    SetThreadPriority(THREAD_PRIORITY_NORMAL);
                    LogPrint("debug", "idealgoldv2Miner:\n");
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrint("debug", "Threadidealgoldv2Miner() exception");
    }

    LogPrint("debug", "Threadidealgoldv2Miner exiting\n");
}
