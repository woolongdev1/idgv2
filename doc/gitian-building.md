...
*Setup instructions for a Gitian build of idealgoldv2 Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the idealgoldv2
...
- [Building idealgoldv2 Core](#building-bitcoin-core)
...
------------------------------------------------
The instructions below use the automated script [gitian-build.py](https://github.com/idealgoldv2chain/idealgoldv2/blob/master/contrib/gitian-build.py) which only works in Debian/Ubuntu. For manual steps and instructions for fully offline signing, see [this guide](./gitian-building/gitian-building-manual.md).
...
```bash
cp idealgoldv2/contrib/gitian-build.py .
```
...
```
git clone git@github.com:idealgoldv2chain/gitian.sigs.git
...
export BUILD=0.14.9
gpg --output $VERSION-linux/$NAME/idealgoldv2-linux-$BUILD-build.assert.sig --detach-sig $VERSION-linux/$NAME/bitcoin-linux-$BUILD-build.assert 
gpg --output $VERSION-osx-unsigned/$NAME/idealgoldv2-osx-$BUILD-build.assert.sig --detach-sig 0$VERSION-osx-unsigned/$NAME/bitcoin-osx-$BUILD-build.assert 
gpg --output $VERSION-win-unsigned/$NAME/idealgoldv2-win-$BUILD-build.assert.sig --detach-sig $VERSION-win-unsigned/$NAME/bitcoin-win-$BUILD-build.assert 
```

Make a PR (both the `.assert` and `.assert.sig` files) to the
[idealgoldv2chain/gitian.sigs](https://github.com/idealgoldv2chain/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/idealgoldv2-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/idealgoldv2-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/idealgoldv2-osx-*-build.assert
```
