...
Below are some notes on how to build idealgoldv2 Core for Windows.

The options known to work for building idealgoldv2 Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the idealgoldv2 Core Windows release binaries.
...
Note that for WSL the idealgoldv2 Core source path MUST be somewhere in the default mount file system, for
example /usr/src/idealgoldv2, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/idealgoldv2/idealgoldv2.git
    sudo chmod -R a+rw idealgoldv2
...
way. This will install to `c:\workspace\idealgoldv2`, for example:

    make install DESTDIR=/mnt/c/workspace/idealgoldv2
...
causes two of the idealgoldv2 executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the idealgoldv2 code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the idealgoldv2 source code).
